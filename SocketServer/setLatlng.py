...
for item in globe:
    access.updateCountryPosition(item,globe[item][0],globe[item][1],0)


for item in china:
    name= item['name'].replace('维吾尔自治区','').replace('回族自治区','').replace('壮族自治区','').replace('自治区','').replace('省','').replace('市','')
    access.updateCountryPosition(name,item['log'],item['lat'],1)
    for city in item['children']:
        access.updateCountryPosition(city['name'],city['log'],city['lat'],2)